function setListners() {
  ...
  socket.on('updatefile-res', (data) => {
    ...
    if (message === 'success') {
      ...
      if (fileList.findIndex((f) => f.fname === fname) === activeIndex) editor.setValue(content);
  ...
  socket.on('createfile-res', (data) => {
    ...
    } else document.getElementById('file-err').innerHTML = 'Server response:' + message;
  });
    ...
    if (message === 'success') {
      ...
      if (code !== null) {
        ...
        document.getElementById(
          'exit-code'
        ).innerHTML = `process exited with error or timeout ${result.stderr}`;
      }
    } else {
      lastOutput = '';
