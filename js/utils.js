function addTab(name, index) {
  ...
  tab.className =
    ...
    (index === activeIndex ? 'bg-blue-100' : 'bg-indigo-300');
...
function disableTab(index) {
  document.getElementById('tab' + index).className =
    'flex flex-row justify-between items-center p-2 bg-indigo-300';
}

function enableTab(index) {
  document.getElementById('tab' + index).className =
    'flex flex-row justify-between items-center p-2 bg-blue-100';
}
...
function setLanguageEditor(fname) {
  if (fname.endsWith('.c')) mode = 'x-c';
  else if (fname.endsWith('.cpp')) mode = 'x-c++src';
  else if (fname.endsWith('.java')) mode = 'x-java';
  else if (fname.endsWith('.py')) mode = 'x-python';
  else mode = 'text';
